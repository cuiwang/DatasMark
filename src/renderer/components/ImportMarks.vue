<template>
  <div>
    <el-dialog title="导入标签" :visible.sync="showDialog" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-form ref="projectForm" :model="markForm" label-width="100px">
        ...
        <el-form-item label="标签类型" required>
          <el-radio-group v-model="markTypeRadio">
            <el-radio :label="0">实体标签</el-radio>
            <el-radio :label="1">关系标签</el-radio>
            <el-radio :label="2" disabled>对话标签</el-radio>
            <el-radio :label="3" disabled>分类标签</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :disabled="markForm.content.length === 0" type="primary" :loading="isReadingFile" @click="saveMarkType">保 存</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      markTypeRadio: 0, //0:entity,1:relation
      markForm: {
        content: [],
      },
      ...
      for (const item of this.markForm.content) {
        ...
        if (item.datas.length > 0) {
          ...
          newMarkTypeForm.content = item.datas
            ...
            .toString();
          // 保存到数据库
          if (this.markTypeRadio === 0) {
            db_utils.insert(db_utils.MARK_TYPES_DB, newMarkTypeForm, (err, newDoc) => {
          ...
          } else if (this.markTypeRadio === 1) {
            // 保存到数据库
            db_utils.insert(db_utils.RELATION_TYPES_DB, newMarkTypeForm, (err, newDoc) => {
              //console.log('marktype new:' + newDoc);
              this.$message({
                message: item.name + ':写入成功!(关系标签)',
                type: 'success',
                center: true,
                offset: 70,
              });
      ...
      if (path) {
        ...
        stream.on('end', () => {
          ...
          if (_data.type === 'entity') {
            this.markTypeRadio = 0;
          } else if (_data.type === 'relation') {
            this.markTypeRadio = 1;
          }
